...
export default function MainPage() {
  ...
  return (
    <div className="pb-16" style={{ background: 'var(--color-background)' }}>
      <div className="max-w-[1440px] mx-auto px-8 pt-8 flex gap-8 items-start">
        ...
        <aside className="w-52 shrink-0 sticky top-24 space-y-3">
          ...
          {showSkeleton
            ? Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="flex items-center gap-3 p-2">
                  <div className="w-10 h-10 rounded-full animate-pulse shrink-0" style={{ background: 'var(--color-surface-container)' }} />
                  <div className="flex-1 space-y-1.5">
                    <div className="h-3 rounded animate-pulse" style={{ background: 'var(--color-surface-container)', width: '70%' }} />
                    <div className="h-2.5 rounded animate-pulse" style={{ background: 'var(--color-surface-container)', width: '40%' }} />
                  </div>
                </div>
              ))
            : popularAuthors.map(author => (
                <Link key={author.id} to={`/profile/${author.nickname}`}
                  className="flex items-center gap-3 p-2 rounded-xl group transition-colors hover:bg-surface-container">
          ...
          <Link to="/gallery/free"
            className="block text-center text-xs font-bold py-2 rounded-lg mt-2 transition-colors hover:text-[var(--color-primary)]"
            ...
            <div className="flex justify-between items-end mb-7">
              <div>
                <span className="block text-xs font-bold tracking-widest uppercase mb-1.5"
                  style={{ color: 'var(--color-primary)' }}>큐레이션 작품</span>
              ...
              {showSkeleton
                ? Array.from({ length: 6 }).map((_, i) => (
                    <div key={i}>
                      ...
                      <div className="h-4 rounded animate-pulse mb-1.5" style={{ background: 'var(--color-surface-container)', width: '60%' }} />
                      <div className="h-3 rounded animate-pulse" style={{ background: 'var(--color-surface-container)', width: '35%' }} />
                    </div>
                  ))
                : visibleTrending.length === 0
                ? <p className="col-span-2 text-center text-sm py-12" style={{ color: 'var(--color-on-surface-variant)' }}>표시할 작품이 없습니다.</p>
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-4">
              {showSkeleton
                ...
                ? <p className="col-span-3 text-center text-sm py-12" style={{ color: 'var(--color-on-surface-variant)' }}>표시할 작품이 없습니다.</p>
                : visibleRecent.map(item => (
                    <Link key={item.postId} to={`/gallery/${item.postId}`}
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-1 p-2">
                        ...
                        <span className="text-xs" style={{ color: 'var(--color-on-surface)' }}>by {item.authorNickname}</span>
                      </div>
                    </Link>
                  ))
              }
        ...
        <aside className="w-56 shrink-0 sticky top-24">
            ...
            {showSkeleton
              ? Array.from({ length: 7 }).map((_, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div className="w-12 h-12 rounded-lg animate-pulse shrink-0" style={{ background: 'var(--color-surface-container)' }} />
                    <div className="flex-1 space-y-1.5">
                      <div className="h-3 rounded animate-pulse" style={{ background: 'var(--color-surface-container)' }} />
                      <div className="h-2.5 rounded animate-pulse" style={{ background: 'var(--color-surface-container)', width: '60%' }} />
                    </div>
                  </div>
                ))
              : visibleHot.length === 0
              ? <p className="text-xs py-4" style={{ color: 'var(--color-on-surface-variant)' }}>표시할 작품이 없습니다.</p>
              : visibleHot.map((item, idx) => (
                  <Link key={item.postId} to={`/gallery/${item.postId}`}
                    className="flex items-center gap-3 group">
                    <span className="text-sm font-bold italic w-4 shrink-0 text-center"
                      style={{ color: idx === 0 ? 'var(--color-primary)' : 'var(--color-on-surface-variant)' }}>
                      {idx + 1}
                    </span>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold truncate group-hover:text-[var(--color-primary)] transition-colors leading-tight">
                        {item.title}
                      </p>
          ...
          <Link to="/gallery/free"
            className="block text-center text-xs font-bold py-2 rounded-lg mt-4 transition-colors hover:text-[var(--color-primary)]"
            style={{ color: 'var(--color-on-surface-variant)', background: 'var(--color-surface-container-low)' }}>
            더 보기 →
          </Link>
        </aside>
