...
export default function CommissionChat({ commissionId, meId, readOnly = false }: Props) {
  ...
  return (
    <div className="rounded-2xl border flex flex-col" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-outline)', height: 480 }}>
      {/* 헤더 */}
      <div className="px-5 py-3 border-b flex items-center gap-2 flex-shrink-0" style={{ borderColor: 'var(--color-outline)' }}>
        <span className="material-symbols-outlined text-base" style={{ color: 'var(--color-primary)' }}>chat</span>
        <span className="font-bold text-sm">채팅</span>
        {otherPresent && (
          <span className="ml-auto flex items-center gap-1 text-xs font-bold" style={{ color: 'var(--color-success)' }}>
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
        {loading ? (
          <div className="flex items-center justify-center h-full">
            <div className="animate-spin rounded-full w-6 h-6 border-2" style={{ borderColor: 'var(--color-primary)', borderTopColor: 'transparent' }} />
          </div>
        ) : messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-center">
            <span className="material-symbols-outlined text-3xl" style={{ color: 'var(--color-outline)' }}>forum</span>
            <p className="text-sm" style={{ color: 'var(--color-on-surface-variant)' }}>아직 메시지가 없습니다.</p>
            <p className="text-xs" style={{ color: 'var(--color-outline-strong)' }}>먼저 인사를 건네보세요.</p>
          </div>
        ) : (
          <>
            {hasMore && (
            ...
            return (
              <div key={m.messageId} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] flex flex-col ${mine ? 'items-end' : 'items-start'}`}>
                  {!mine && (
                    <span className="text-xs mb-0.5" style={{ color: 'var(--color-on-surface-variant)' }}>
                      {m.senderNickname ?? '상대'}
                    </span>
                  )}
                  <div className="px-3 py-2 rounded-2xl text-sm whitespace-pre-wrap break-words"
                    style={mine
                      ...
                      : { background: 'var(--color-surface-container)', color: 'var(--color-on-surface)', borderTopLeftRadius: 4 }}>
                    {m.content}
                  </div>
                  <span className="text-xs mt-0.5 flex items-center gap-1" style={{ color: 'var(--color-outline-strong)' }}>
                    {mine && m.isRead && <span style={{ color: 'var(--color-primary)' }}>읽음</span>}
      ...
      {readOnly ? (
        ...
        <form onSubmit={handleSend} className="px-3 py-3 border-t flex items-center gap-2 flex-shrink-0" style={{ borderColor: 'var(--color-outline)' }}>
          <input
            value={input}
