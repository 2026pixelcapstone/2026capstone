    ...
    <button type="button" onClick={onClick}
      ...
      {conv.partnerProfileImageUrl ? (
        ...
        <span className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0"
          ...
          <span className="material-symbols-outlined text-lg" style={{ color: 'var(--color-on-surface-variant)' }}>person</span>
        </span>
      )}
      <span className="flex-1 min-w-0">
        <span className="flex items-center gap-2">
          ...
          {conv.lastMessageAt && (
            <span className="text-xs flex-shrink-0" style={{ color: 'var(--color-on-surface-variant)' }}>{timeAgo(conv.lastMessageAt)}</span>
          )}
        </span>
